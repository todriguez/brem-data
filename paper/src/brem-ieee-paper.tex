...
For DeFi, the BREM framework offers three practical applications. First, \textbf{pre-deployment risk scoring}: new protocols can be evaluated against the 9-cell framework before launch to identify whether the design places the project above or below the 2.5 failure threshold. Second, \textbf{systemic risk monitoring}: sm provides a measurable indicator of governance centralization; protocols with sm $\geq$ 3 are structurally vulnerable to the sf$\rightarrow$sm cascade when economic conditions deteriorate. Third, \textbf{responsible governance design}: the framework provides empirical evidence that protocol immutability is achievable (Uniswap V2) and that retained discretion is exploitable under economic stress (Terra/Luna).
...
Future work includes: (1) expanding the DeFi dataset; (2) developing automated scoring tools that derive BREM scores from on-chain governance data; (3) longitudinal tracking of scores to identify early warning signals; and (4) formal modeling of cascade propagation mechanisms. The 21 unresolved projects provide a natural out-of-sample validation cohort.
